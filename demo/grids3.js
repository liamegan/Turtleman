...
for (let i = 0; i <= gridW; i++) {
  for (let j = 0; j <= gridH; j++) {
    points[i].push({
      x: (i + randomBetween(-offset, offset)) * gridWFactor,
      y: (j + randomBetween(-offset, offset)) * gridHFactor,
    });
  }
}
...
grids.forEach((grid) => {
  // toy.jump(grid.points[0].x, grid.points[0].y);
  // toy.goto(grid.points[1].x, grid.points[1].y);
  // toy.goto(grid.points[2].x, grid.points[2].y);
  // toy.goto(grid.points[3].x, grid.points[3].y);
  // toy.goto(grid.points[0].x, grid.points[0].y);
  ...
  for (let i = 0; i < splits; i++) {
    const t = i / splits;
    const topEdgePoint = {
      x: lerp(grid.points[0].x, grid.points[1].x, t),
      y: lerp(grid.points[0].y, grid.points[1].y, t),
    };
    const leftEdgePoint = {
      x: lerp(grid.points[0].x, grid.points[3].x, t),
      y: lerp(grid.points[0].y, grid.points[3].y, t),
    };

    if (edgeDirection === 1) {
      // Need to switch the points for lerping the opposite edge
      const bottomEdgePoint = {
        x: lerp(grid.points[2].x, grid.points[3].x, t),
        y: lerp(grid.points[2].y, grid.points[3].y, t),
      };
      const rightEdgePoint = {
        x: lerp(grid.points[2].x, grid.points[1].x, t),
        y: lerp(grid.points[2].y, grid.points[1].y, t),
      };

      toy.jump(topEdgePoint.x, topEdgePoint.y);
      toy.goto(rightEdgePoint.x, rightEdgePoint.y);

      toy.jump(bottomEdgePoint.x, bottomEdgePoint.y);
      toy.goto(leftEdgePoint.x, leftEdgePoint.y);
    } else {
      const bottomEdgePoint = {
        x: lerp(grid.points[3].x, grid.points[2].x, t),
        y: lerp(grid.points[3].y, grid.points[2].y, t),
      };
      const rightEdgePoint = {
        x: lerp(grid.points[1].x, grid.points[2].x, t),
        y: lerp(grid.points[1].y, grid.points[2].y, t),
      };
